...
const MasonryList = (props: { filterTags: string[], type: string }) => {
    ...
    const {
        ...
        "/cards"
        + "?type=" + props?.type + (props?.filterTags.length > 0 ? "&withTags=" + encodeURIComponent(props?.filterTags.join(",")) : "")
        + "&page=" + page,
        { method: "GET" }
    );
    ...
    return (
        noCards ? <InfiniteScrollEnd type={props?.type} noCards={true} /> : <InfiniteScroll
            className="!overflow-hidden"
            dataLength={cardData.length}
            next={() => { getData(); setPage(page + 1); }}
            hasMore={hasMore}
